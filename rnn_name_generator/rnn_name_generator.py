...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    ids=[[char_to_id[char]  for char in name] for name in seqs]
    all_seqs = [np.pad(id, (0, max_len - min(20,len(id))))[:max_len] for id in ids]
...
class RNNLM(nn.Module):
    ...
    def forward(self, x, h_last=None):
        ...
        B,S,G=out.size(0),out.size(1), out.size(2)
        out = out.reshape(B*S, G)
        out = self.linear(self.relu(out))
        out=out.reshape(B,S,self.vocab_size)
        return out, h
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        ...
        for i in range(0, num_batches):
            inputs = Xtrain[i * batch_size:(i + 1) * batch_size, :]
            targets = Ytrain[i * batch_size:(i + 1) * batch_size, :]
            h = torch.zeros(1, batch_size, model.gru_size)
            outputs, h = model(inputs, h)
            loss = lossfn(outputs.permute(0, 2, 1), targets)
            optim.zero_grad()
            loss.backward()
            optim.step()
        val_loss = calc_val_loss(model, Xval, Yval)
        print('Epoch: [{}/{}], loss: {:.4f}, val_loss: {:.4f}.'.format(e + 1, max_epoch, loss.item(), val_loss))
        pass
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        output, h = model(x, h)
        if sample:
            id = torch.multinomial(sm(output[0]), 1)[0][0].tolist()
        else:
            id = output[0].detach().cpu().argmax(axis=1).numpy().tolist()[0]
        char = id_to_char[id]
        if char == '.':
            break
        else:
